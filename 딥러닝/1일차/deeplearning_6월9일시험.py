...
# train(훈련세트)와 test(테스트세트) 7:3으로 분류
...
# drop4 = layers.Dropout(0.4)  # overfit 감소 40%  요고 주석하면 과적합 예방안된다!!
dense2 = layers.Dense(10, activation='softmax', name="output")
# model = models.Sequential([dense1, dense3, drop4, dense2]) 요고 주석하면!!
model = models.Sequential([dense1, dense3, dense2])  #위에꺼 두개 주석하고 이ㅓㄱ실행한거당!!
...
손실 함수 (loss): 손실 함수는 값을 예측하려할 때 데이터에대한 예측값과 실제의 값을 비교하는 함수로 모델을 훈련시킬 때 
        ...
        mse(mean squared eror 평균제곱오차, mae mean absolute error 평균절대오차) : 회귀문제, 
        시퀀스 학습 문제: CTC(Connection Temporal Classification) 언어인식)
...
wrong_result = []
[wrong_result.append(n) for n in range(0, len(test_labels)) if predicted_labels[n] != test_labels[n]]
